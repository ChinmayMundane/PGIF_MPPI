...
1. **Predictive Gradient Interaction Field (PGIF) / Gaussian-weighted MPPI (GMPPI)**: Plans trajectories by predicting future human motions and penalizing rollouts via a 2D anisotropic Gaussian cost field elongated along each human's heading vector (motion-cone danger zones).
...
* **`mppi_dynamic_humans.py`**: The core simulation and planning script.
